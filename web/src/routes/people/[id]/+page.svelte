...
<div class="overflow-none h-screen w-screen bg-[#ffecc9] p-2">
	...
	<h1 class="mt-10 mb-5 text-xl text-[#241802] md:text-5xl">{data.person.fullName}</h1>

	{#if data.person.birthYear || data.person.birthCity || data.person.birthCountry}
		<div class="flex items-center gap-2 text-[#241802]">
			<svg
				class="size-5 fill-[#241802] md:size-7"
				...
				/></svg
			>
			{#if data.person.birthYear}
				<p class="text-sm md:text-base">
					Born
					<!-- {months[data.person.birthMonth - 1]} -->
	...
	{#if data.relations.find((r) => (r.relation === 'mother' || r.relation === 'father') && r.person1Id === data.person.id)}
		<div class="mt-10">
			<h2 class="mb-2 text-lg text-[#241802] md:text-xl">Parents</h2>
